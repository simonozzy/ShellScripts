...
LOGFILE=${LOGDIR}/${JOB}_${RUNDATE}.log ; > ${LOGFILE}
...
echo "######    Calling Sequence/Job: ##  ${JOBNAME} ## - any warning and fatal messages: ######" | tee -a ${LOGFILE} # tee -a

START_ID=`dsjob  -logsum -type STARTED ${PROJECTNAME} ${DS_JOB} 2> /dev/null | gawk 'ORS=(FNR%2)?FS:RS' | grep Starting | tail -1 | awk '{print $1 }'`
WARN_IDS=`dsjob  -logsum -type WARNING ${PROJECTNAME} ${DS_JOB} 2> /dev/null | grep WARNING | awk '{print $1 }' ` 
FATAL_IDS=`dsjob -logsum -type FATAL   ${PROJECTNAME} ${DS_JOB} 2> /dev/null | grep FATAL | awk '{print $1 }'`
# INFO_IDS=`dsjob  -logsum -type INFO    ${PROJECTNAME} ${DS_JOB} 2> /dev/null | grep "(JobControl.+status = 3.+Aborted)|Unhandled abort|(some_other_pattern)" | awk '{print $1 }'` 
...
for TEST_ID in ${ALL_IDS}
 do
   ...
   then
      WARN_DTL=`dsjob -logdetail ${PROJECTNAME} ${DS_JOB} ${TEST_ID} 2> /dev/null`  
      echo ${WARN_DTL}                                                >> ${LOGFILE}
   fi
 done

tail -50 ${LOGFILE} | egrep -v "^#"
...
START_ID=`dsjob  -logsum -type STARTED ${PROJECTNAME} ${DS_JOB} | gawk 'ORS=(FNR%2)?FS:RS' | grep Starting | tail -1 | awk '{print $1 }' 2>&1 > /dev/null`
WARN_IDS=`dsjob  -logsum -type WARNING ${PROJECTNAME} ${DS_JOB} | grep WARNING | awk '{print $1 }' 2>&1 > /dev/null`
FATAL_IDS=`dsjob -logsum -type FATAL   ${PROJECTNAME} ${DS_JOB} | grep FATAL | awk '{print $1 }' 2>&1 > /dev/null`
...
for TEST_ID in ${ALL_IDS}
 do
   if [[ "${TEST_ID}" -gt "${START_ID}" ]]
   then
      WARN_DTL=`dsjob -logdetail ${PROJ_DEF} ${DS_JOB} ${TEST_ID} 2> /dev/null` 
      echo ${WARN_DTL}                                               >> ${LOGFILE}
   fi
 done
...
############ DEBUG for single job call ###############

DS_JOB=jb_CDR_CORE_NADB_Ext
...
ALL_IDS=`echo "${WARN_IDS} ${FATAL_IDS} ${INFO_IDS}"` # 
echo ${PROJECTNAME} ${DS_JOB} ${START_ID} ${ALL_IDS}
